...
export function runPending({ onApply = () => {} } = {}) {
  ...
  for (const m of pendingMigrations()) {
    ...
    // SQLite's table-rebuild procedure needs PRAGMA foreign_keys = OFF, and
    // that pragma is a silent no-op inside a transaction -- the rebuild would
    // then fail, or worse, succeed while dropping child rows. A migration that
    // needs it says so, and takes responsibility for its own atomicity.
    if (/^--\s*migrate:no-transaction/m.test(sql)) {
      ...
      try {
        ...
        // A rebuild that left a dangling reference is worse than a failed
        // migration, because nothing downstream would notice.
        const violations = db.prepare("PRAGMA foreign_key_check").all();
        if (violations.length > 0) {
          throw new Error(
            `${m.name} left ${violations.length} foreign key violation(s): ` +
              violations.slice(0, 3).map((v) => `${v.table}.${v.rowid}`).join(", "),
          );
        }
      }
    } else {
  ...
  // Stamped whenever the ledger is complete, NOT only when something ran.
  //
  // Guarded on `applied.length > 0`, this could never catch up after a
  // migration was applied and the version constant bumped afterwards -- which
  // is the ordinary order of doing it: write the file, run it, then update the
  // constant. The next run finds nothing to apply, skips the stamp, and the
  // database is left claiming a version below what the code expects.
  //
  // assertSchemaCurrent then refuses to start and tells the user to run
  // migrations, which reports "up to date" and changes nothing. The only exit
  // is editing app_settings by hand, on a machine whose app will not boot.
  //
  // The stamp means "this database matches this code version", so the
  // condition for writing it is that the ledger covers everything the code
  // knows about -- not that this particular run did some work.
